suppressPackageStartupMessages({
  library(gmrfs); library(INLA); library(Matrix)
})

set.seed(1)
n <- 20; p <- 2

ar1_cov <- function(n, rho) outer(seq_len(n), seq_len(n),
                                  function(i, j) rho^abs(i - j))
G_list  <- list(ar1_cov(n, 0.5), ar1_cov(n, 0.2))
R_list  <- lapply(G_list, function(G) chol2inv(chol(G)))

## sblocks: latent dim p*n with block-diagonal precision.
cm  <- sblocks(n = n, p = p, R_list = R_list, tau0 = 1.0)

## Each obs i maps to (i, n+i, ..., (p-1)*n + i).
y   <- rnorm(n, 0, 0.5)
A   <- sparseMatrix(i    = rep(seq_len(n), p),
                    j    = as.vector(outer(seq_len(n), (0:(p - 1)) * n, `+`)),
                    x    = 1.0,
                    dims = c(n, p * n))
stk <- inla.stack(data    = list(y = y),
                  A       = list(A, 1),
                  effects = list(idx = seq_len(p * n),
                                 intercept = rep(1, n)),
                  tag     = "obs")

m <- INLA::inla(y ~ -1 + intercept + f(idx, model = cm),
                data              = inla.stack.data(stk),
                control.predictor = list(A = inla.stack.A(stk)),
                family            = "gaussian",
                control.inla      = list(int.strategy = "eb"))

print(m$summary.hyperpar)
stopifnot(isTRUE(m$ok))
cat("\n\n[OK] sblocks minimal demo ran successfully.\n")
